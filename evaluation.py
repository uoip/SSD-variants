...
def eval_voc_detection(
    ...
    prec, rec = calc_voc_detection_pr(
        pred_bboxes, pred_labels, pred_scores, gt_bboxes, gt_labels,
        gt_difficults, iou_thresh=iou_thresh)

    ap = calc_voc_detection_ap(prec, rec, use_07_metric=use_07_metric)

    return ap, np.nanmean(ap)



def calc_voc_detection_pr(
    pred_bboxes, pred_labels, pred_scores, gt_bboxes, gt_labels,
    ...
    for pred_bbox, pred_label, pred_score, gt_bbox, gt_label, gt_difficult in zip(
        ...
        for l in np.unique(np.concatenate((pred_label, gt_label)).astype(int)):
            ...
            if len(pred_bbox_l) == 0:
                continue
            if len(gt_bbox_l) == 0:
                match[l].extend((0,) * len(pred_bbox_l))
                continue
            ...
            for gt_idx in gt_index:
                if gt_idx >= 0:
                    ...
                    match[l].append(0)
            #print(len(pred_bbox_l), len(pred_bbox_l), len(score[l]), len(match[1]))
...
def calc_voc_detection_ap(prec, rec, use_07_metric=False):
    n_fg_class = len(prec)
    ...
    for l in range(n_fg_class):
        ...
        if use_07_metric:
            ...
            ap[l] = np.sum((mrec[i+1] - mrec[i]) * mrec[i+1])
